...
int main(int argc, char** argv) try
{
    ...
    if (argc != 4)
    {
        cout << "Give the path to the examples/faces directory as the argument to this, as well as the number of xmls for training and number of overall runs" << endl;
        cout << "program.  For example, if you are in the examples folder then execute " << endl;
        cout << "this program by running: " << endl;
        cout << "   ./dnn_mmod_ex dir numberOfXMLs runs" << endl;
        cout << endl;
        return 0;
    }
    ...
    // configurations to vary for each run:
    int long_dim[] = {30, 40, 50, 60, 70, 80, 80, 80, 80, 80};
    int short_dim[] = {20, 30, 40, 50, 60, 30, 40, 50, 60, 70};

    // best ones for smile: 1, 2, 5, 6
    //           40x30, 50x40, 80x30, 80x40

    while (runs < atoi(argv[3])) {

        int fileCount = 0;

        while (fileCount < atoi(argv[2])) {

            log("log" + to_string(runs) + ".txt", "Started: " + getTime() + '\n');
            ...
            mmod_options options(face_boxes_train, long_dim[runs], short_dim[runs]); // face: 80x80   -   smile: 70, 30
            ...
            trainer.be_verbose();
            trainer.set_synchronization_file("mmod_sync" + to_string(runs), std::chrono::minutes(5));
            ...
            cropper.set_min_object_size(long_dim[runs], short_dim[runs]);
            ...
            fstream f1, f2;
            f1.open("metrics" + to_string(runs) + ".csv", ios::out | ios::app);
            f2.open("results" + to_string(runs) + ".txt", ios::out | ios::app);
            ...
            net.clean();
            serialize("mmod_network" + to_string(runs) + ".dat") << net;
            ...
            log("log" + to_string(runs) + ".txt", "Finished: " + getTime() + '\n');
            ...
            log("log" + to_string(runs) + ".txt", "Learning rate: " + to_string(trainer.get_learning_rate()) + " " + "Training Loss: " + to_string(trainer.get_average_loss()) + " " + "Steps: " + to_string(trainer.get_train_one_step_calls()) + '\n');
