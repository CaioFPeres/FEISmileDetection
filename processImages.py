...
def createDoc():
    ...
    commentInnerText = newDocRoot.createTextNode("Annotated faces from FEI dataset")
...
if __name__ == "__main__":
    ...
    if(not os.path.exists("fei_face/cropped")):
        os.mkdir("fei_face/cropped")

    DOMTree = minidom.parse("./training.xml")
    ...
    for imageElem in collection.getElementsByTagName("images")[0].childNodes:
        if(imageElem.nodeType != imageElem.TEXT_NODE):
            ...
            if(count >= int(sys.argv[1])):
                ...
                fileName = imageElem.getAttribute("file").split("\\")[2]
                top = int(imageElem.getElementsByTagName("box")[0].getAttribute("top"))
                left = int(imageElem.getElementsByTagName("box")[0].getAttribute("left"))
                width = int(imageElem.getElementsByTagName("box")[0].getAttribute("width"))
                height = int(imageElem.getElementsByTagName("box")[0].getAttribute("height"))

                im = Image.open("fei_face/b/" + fileName)
                ...
                try:
                    newImg.save("fei_face/cropped/" + newName + ".png")
                except:
                ...
                image.setAttribute('file', "cropped/" + newName + ".png")
                image.setAttribute('width', str(newImg.size[0]))
                image.setAttribute('height', str(newImg.size[1]))
                
                # add box
                box = newDocRoot.createElement('box')
                label = newDocRoot.createElement('label')
                box.appendChild(label)
                
                # add label
                labelInnerText = newDocRoot.createTextNode("smile")
                label.appendChild(labelInnerText)
    ...
    with open("fei_face/training.xml", "w") as f:
        f.write(training)